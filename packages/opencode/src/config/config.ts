...
import { Context, Duration, Effect, Exit, Fiber, Layer, Option, Scope } from "effect"
...
export const layer = Layer.effect(
  ...
  Effect.gen(function* () {
    ...
    const state = yield* InstanceState.make<State>((ctx) =>
      loadInstanceState(ctx).pipe(Effect.orDie) as unknown as Effect.Effect<State, never, Scope.Scope>,
    )
